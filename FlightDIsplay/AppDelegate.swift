...
//        wSocket = SRWebSocket(URL: NSURL(string: "ws://voha-bbb.linuxdrone.org:7681/xxx"))
    ...
    var managedObjectModel: NSManagedObjectModel {
        if !_managedObjectModel {
            let modelURL = NSBundle.mainBundle().URLForResource("FlightDIsplay", withExtension: "momd")
    ...
    var persistentStoreCoordinator: NSPersistentStoreCoordinator {
        if !_persistentStoreCoordinator {
            let storeURL = self.applicationDocumentsDirectory.URLByAppendingPathComponent("FlightDIsplay.sqlite")
            var error: NSError? = nil
